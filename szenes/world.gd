...
func _ready() -> void:
	set_tile(groundLayer, Vector2i(0, 0), source, grass)
...
func get_current_chunk() -> Vector2:
	...
	var position = get_tile_of_layer(groundLayer, camera)
...
func get_tile_of_layer(layer: TileMapLayer, position: Vector2):
	var isometric_width = ((8 * TILE_WIDTH) + (7 * TILE_HEIGHT)) / CHUNK_SIZE
	var isometric_x = (position.x / isometric_width) + (position.y / TILE_HEIGHT)
	var isometric_y = (position.y / TILE_HEIGHT) - (position.x / isometric_width)
	return Vector2(floor(isometric_x), floor(isometric_y))

func curser():
	var mouse = get_global_mouse_position()
	var tile = get_tile_of_layer(groundLayer, mouse)
	debug_log.text += "Curser: " + str(tile) + "\n"
	
	if tile != curser_tile:
		if original_tile != null:
			groundLayer.set_cell(curser_tile, source, original_tile)
		
		original_tile = groundLayer.get_cell_atlas_coords(tile)
		groundLayer.set_cell(tile, source, forest)
